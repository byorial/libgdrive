...
# sjva 공용
from framework import socketio, SystemModelSetting, app
...
try:
    ...
    os.system("pip install --upgrade httplib2 google-api-python-client google-auth-httplib2 google-auth-oauthlib")
    import pickle
    from googleapiclient.discovery import build
    from google_auth_oauthlib.flow import InstalledAppFlow
    from google.auth.transport.requests import Request
...
class LibGdrive:
    ...
    def sa_authorize(cls, json_path, return_service=False, scopes=None, impersonate=None):
        if not os.path.exists(json_path):
            logger.debug('can not recognize gdrive_auth_path(%s)', json_path)
    ...
    def sa_authorize_for_multiple_connection(cls, json_path, max_connection):
        ...
        if os.path.isdir(json_path):
            ...
            logger.debug('need multiple json files(%s)', json_path)
    ...
    def auth_by_rclone_remote(cls, remote, set_service=False):
        try:
            try:
                ...
                os.system("pip install google-oauth")
                from google.oauth2.credentials import Credentials as OAuth2Credentials
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    logger.info('credential expired. refresh')
                    creds.refresh(Request())
